...
const estilos = {
  ...
  input: { width: "160px", padding: "7px", borderRadius: "4px", border: "1px solid #e5e7eb", background: "#223446ff", fontSize: "0.96rem", color: "#fff" },
...
export default function Tarifas() {
  ...
  const cargarTiposVehiculo = async () => {
    try {
      const res = await axios.get("http://localhost:3000/api/vehiculo-tipos", { withCredentials: true });
      const mapa = {};
      res.data.forEach((t) => {
  ...
  return (
    <div style={estilos.contenedor}>
      ...
      <button style={estilos.botonesuperior} onClick={() => { setAgregando(!agregando); }}>
        ...
        <form style={estilos.formCont} onSubmit={handleAddTarifa}>
          ...
          <div style={{ position: "relative" }}>
            ...
            {resultadosFiltrados.length > 0 && (
              <div style={{ position: "absolute", background: "#1e293b", border: "1px solid #38bdf8", width: "200px", zIndex: 10 }}>
                {resultadosFiltrados.map((item) => (
                  <div
                    key={item.id}
                    onClick={() => {
                      setNuevaTarifa({ ...nuevaTarifa, vehiculo_tipo_id: item.id });
                      setBusquedaTipo(item.nombre);
                      setResultadosFiltrados([]);
                    }}
                    style={{ padding: "6px", cursor: "pointer", color: "white", borderBottom: "1px solid #334155" }}
                  >
                    {item.id} - {item.nombre}
                  </div>
                ))}
          ...
          <input name="precio_hora" type="number" value={nuevaTarifa.precio_hora} onChange={(e) => setNuevaTarifa({ ...nuevaTarifa, precio_hora: e.target.value })} placeholder="Precio hora" style={estilos.input} />
          <input name="precio_dia" type="number" value={nuevaTarifa.precio_dia} onChange={(e) => setNuevaTarifa({ ...nuevaTarifa, precio_dia: e.target.value })} placeholder="Precio día" style={estilos.input} />
          <input name="precio_mes" type="number" value={nuevaTarifa.precio_mes} onChange={(e) => setNuevaTarifa({ ...nuevaTarifa, precio_mes: e.target.value })} placeholder="Precio mes" style={estilos.input} />
          <input name="descripcion" type="text" value={nuevaTarifa.descripcion} onChange={(e) => setNuevaTarifa({ ...nuevaTarifa, descripcion: e.target.value })} placeholder="Descripción" style={estilos.nombre} />

          <button style={estilos.boton} type="submit">Guardar</button>
          <button style={estilos.botonEditar} type="button" onClick={() => setAgregando(false)}>Cancelar</button>
        </form>
      )}
        ...
        <tbody>
          {tarifas.map((t) => (
            <tr key={t.tarifa_id}>
              ...
              <td style={estilos.td}>
                <button style={estilos.boton} onClick={() => handleDelete(t.tarifa_id)}>🗑️</button>
                <button style={estilos.botonEditar} onClick={() => handleEdit(t)}>✏️</button>
              </td>
            </tr>
          ))}
        </tbody>
